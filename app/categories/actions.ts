...
export async function createCategoryAction(formData: FormData) {
  ...
  if (!parsed.success) {
    redirect(`/categories/new?error=${encodeURIComponent(parsed.error.issues[0]?.message ?? "Invalid data.")}`);
  }
...
export async function updateCategoryAction(id: string, formData: FormData) {
  ...
  if (!parsed.success) {
    redirect(`/categories/${id}/edit?error=${encodeURIComponent(parsed.error.issues[0]?.message ?? "Invalid data.")}`);
  }
...
export async function deleteCategoryAction(formData: FormData) {
  ...
  try {
    ...
    if (productCount > 0) {
      redirect("/categories?error=Category%20cannot%20be%20deleted%20while%20products%20still%20reference%20it");
    }
